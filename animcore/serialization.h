...
namespace Serialization
{
	...
	private:
		inline bool CheckSharedPtrSaved(const void* obj, uint64_t& addrOut)
		{
			auto iter = m_SharedPtrQueue.find(obj);
			if (iter == m_SharedPtrQueue.end())
			{
				m_SharedPtrQueue[obj] = ++m_NextAddr;
				addrOut = m_NextAddr;
				return false;
			}
			addrOut = iter->second;
		...
		uint64_t m_NextAddr;
		std::unordered_map<const void*, uint64_t> m_SharedPtrQueue;
	...
	private:
		void SetConcreteSharedPtrReference(uint64_t refValue, void* concreteValue)
		{
			auto iter = m_SharedPtrPatchupMap.find(refValue);
			if (iter == m_SharedPtrPatchupMap.end())
			{
				SharedPtrPatch patch;
				patch.m_ConcreteAddr = concreteValue;
				m_SharedPtrPatchupMap[refValue] = std::move(patch);
			}
			else
			{
				ANIM_ASSERT(iter->second.m_ConcreteAddr == nullptr);
				iter->second.m_ConcreteAddr = concreteValue;
			}
		}

		void SetAbstractSharedPtrReference(uint64_t refValue, std::function<void(void*)> patchLambda)
		{
			auto iter = m_SharedPtrPatchupMap.find(refValue);
			if (iter == m_SharedPtrPatchupMap.end())
			{
				SharedPtrPatch patch;
				patch.m_ConcreteAddr = nullptr;
				patch.m_PatchLambdas.Push(patchLambda);
			}
			else
			{
				iter->second.m_PatchLambdas.Push(patchLambda);
			}
		}

		struct SharedPtrPatch
		{
			void * m_ConcreteAddr;
			Array<std::function<void(void *)>> m_PatchLambdas;
		};
		std::unordered_map<uint64_t, SharedPtrPatch> m_SharedPtrPatchupMap;
...
#pragma region SharedPtr
		...
		struct SerializeHelper<SharedPtr<T>>
		{
			...
			{
				static_assert(false, "Do not support serialization of SharedPtr");
			}
		};
